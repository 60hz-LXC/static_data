...
app.post('/api/saveGreenData', (req, res) => { 
  let { ip_address, region, country, browser, operating_system, language, screenWidth, screenHeight, timezoneOffset, referrer } = req.body; 
  referrer = referrer.substring(referrer.lastIndexOf('/')+1); // Get the page slug 
  console.log("Received request with data:", req.body);

  // Check if the same IP has visited within the last two hours 
  const checkSql = "SELECT * FROM green_data WHERE ip_address = ? AND timestamp > DATE_SUB(NOW(), INTERVAL 2 HOUR)";
  ...
  db.query(checkSql, [ip_address], (err, result) => {
    ...
    if (result.length > 0) {
      // If the IP has visited within the last two hours, update the record
      const updateSql = "UPDATE green_data SET pageViews = pageViews+1, pageReferrers = CONCAT(pageReferrers, ' -> ', ?) WHERE ip_address = ?";
      ...
      // If the IP hasn't visited within the last two hours or is new, create a new record
      const insertSql = "INSERT INTO green_data (ip_address, region, country, browser, operating_system, language, screenWidth, screenHeight, timezoneOffset, referrer, pageViews, pageReferrers) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)";
      db.query(insertSql, [ip_address, region, country, browser, operating_system, language, screenWidth, screenHeight, timezoneOffset, referrer, referrer], (err, result) => {
        if (err) {
          console.error("Error inserting to the database:", err);
          res.status(500).json({ message: "Error inserting to the database" });
          ...
          res.json({ message: "Inserted a new record successfully" });
        }
      });
    }
  });
